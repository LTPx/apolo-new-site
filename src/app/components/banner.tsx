        ...
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          {buttons.map((btn, index) => (
            <Link
              href={btn.link}
              key={index}
              className="block w-full rounded bg-primary px-12 py-3 text-sm font-medium text-white shadow hover:bg-primary-600 focus:outline-none focus:ring active:bg-primary-400 sm:w-auto"
            >
              {btn.title}
            </Link>
          ))}
